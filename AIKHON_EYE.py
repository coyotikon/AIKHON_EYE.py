...
def scanner(u):
 ...
 spece=" "*20
 print(f"{g}#"*126)
 print(f"{g}# {r}MIDIA SOCIAL   {g}|        {r}USUARIO {g}        | {r}STATUS CODE{g} | {r}                   LINK   {g}      {spece}                   #")
...
def main(usernames):
 system("clear")
 print(banner)
 print(f"{r}NOTE: Usado para fins educacionais!\n")
 print(f"{y}NOTE: A BUSCA PODE SER FEITA COM:   NOMES,  CPF,  CNPJ  e  NUMEROS DE TELEFONE.")
 print(f"{r}NOTE: for educational purpose only!\n")
 if usernames == []:
  usernames=input(f"{y}Coloque o nome de usuario : {r}:{g}").split()
